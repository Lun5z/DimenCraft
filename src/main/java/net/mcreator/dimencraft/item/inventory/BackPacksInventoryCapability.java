...
public class BackPacksInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	@SubscribeEvent
	@OnlyIn(Dist.CLIENT)
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == DimencraftModItems.BACK_PACKS.get()) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(18) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != DimencraftModItems.BACK_PACKS.get();
			}
